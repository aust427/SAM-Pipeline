...
# totals = h5py.File(input_path + "/totals.hdf5", 'r')
...
header.attrs.create('Ngroups_ThisFile', data=haloprop.shape[0], dtype='uint32')  # how many parent halos in this file
header.attrs.create('Nsubgroups_ThisFile', data=galprop.shape[0], dtype='uint32')  # how many subhalos there are in this file 
header.attrs.create('Nsubvolumes', data=n_sub**3, dtype='int32') # how many subvolumes 

redshift_table = header.create_dataset("Redshifts", (redshifts.shape[0], ),
                                     dtype='<f8', data = redshifts)
...
# subhalo_table_total = header.create_dataset('Nsubgroups_Total_Redshift', (redshifts.shape[0],),
#                                        data=totals['Total_Subgroups'], dtype='<i8')

# halo_table_total = header.create_dataset('Ngroups_Total_Redshift', (redshifts.shape[0],),
#                                       data=totals['Total_Groups'], dtype='<i8')

subhalos = group.create_group("Galprop")

# Velocity & Position 3D 
subhalovel = subhalos.create_dataset("GalpropVel", (galprop.shape[0], 3), 
                                          dtype='<f4', data = galprop[['vx', 'vy', 'vz']].values)
subhalopos = subhalos.create_dataset("GalpropPos", (galprop.shape[0], 3), 
                                          dtype='<f4', data = galprop[['x_position', 'y_position', 'z_position']].values)
